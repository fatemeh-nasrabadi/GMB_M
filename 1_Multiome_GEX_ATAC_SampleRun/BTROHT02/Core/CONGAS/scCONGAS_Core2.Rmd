...
mybarcode_list <- read.table("/home/fatemeh.nasrabadi/ROVIGO_congas/BTROHT02/Core/CONGAS/BTROHT02_core_barcodes_all.tsv", sep="\t", header=TRUE)
mybarcode_list
...
metadata_atac <- filtered_barcodes %>%
  mutate(cell = paste0(X, "-ATAC"), type = UMAP.of.ATAC.in.BTROH02.Core) %>%
  select(cell, type)

# Step 3: Generate Metadata with "-RNA"
metadata_rna <- filtered_barcodes %>%
  mutate(cell = paste0(X, "-RNA"), type = UMAP.of.ATAC.in.BTROH02.Core) %>%
  select(cell, type)
...
```{r}
mysegs <- read.csv("/home/fatemeh.nasrabadi/ROVIGO_congas/BTROHT02/Core/CONGAS/BTROHT02_copre_segs_for_congas.csv", sep = "\t", row.names = 1)
...
myx$input$dataset <- as.data.frame(myx$input$dataset)
myx$input$normalisation <- as.data.frame(myx$input$normalisation)
myx$input$segmentation <- as.data.frame(myx$input$segmentation)
myx$input$metadata <- as.data.frame(myx$input$metadata)
...
getwd()
write.csv(fit_filt$best_fit$cluster_assignments, file = "/home/fatemeh.nasrabadi/ROVIGO_congas/BTROHT02/Core/CONGAS/BTROHT01_CONGAS_Clusters.csv", row.names = FALSE)
```

```{r}
getwd()
saveRDS(fit_filt, "/home/fatemeh.nasrabadi/ROVIGO_congas/BTROHT02/Core/CONGAS/rcongas_obj_filtered_BTROHT02_core_all_cells.rds")
```
...
write.table(df, file = "/home/fatemeh.nasrabadi/ROVIGO_congas/BTROHT02/Core/CONGAS/ploidy_by_segments_BTROHT01_core.tsv", sep = "\t", row.names = FALSE, quote = FALSE)
```
...
sum(is.na(myatac$chr))    # Check for NAs in the 'chr' column
sum(is.na(myatac$from))   # Check for NAs in the 'from' column
sum(is.na(myatac$to))     # Check for NAs in the 'to' column
...
```{r}
# Check for missing values in the required columns
colSums(is.na(myatac[, c("chr", "from", "to")]))
x
